...
class Checker(Visitor):
    ...
    def visit(self, node: Assign, env: Symtab):
        ...
        result = env.get(node.name)
        if result is None:
            self.error(f"Simbolo '{node.name}' no está definido")
    ...
    # Punto 1
    #---------------------------------------------
    def visit(self, node: MathFunc, env: Symtab):
        self.visit(node.name, env)
            
    def visit(self, node: PreInc, env: Symtab):
        self.visit(node.expr, env)

    def visit(self, node: PostInc, env: Symtab):
        self.visit(node.expr, env)
    
    def visit(self, node: PreDec, env: Symtab):
        self.visit(node.expr, env)
    
    def visit(self, node: PostDec, env: Symtab):
        self.visit(node.expr, env)
